...
- The thread should be allowed to remain visible behind the outer edges of the composer; the bottom composer may float as an overlay rather than consuming a dedicated footer slab, as long as the transcript gets enough bottom spacer to keep the latest message clear of the composer.
- On narrow mobile widths, the floating composer may become flat and edge-to-edge with no extra outer side margin, rather than preserving a desktop card silhouette that wastes scarce width.
...
- Mobile text-entry controls in the active composer should use an iPhone-safe font size at or above 16px so focusing the textarea or process selector does not trigger browser auto-zoom.
