...
- adjacent low-signal activity items now collapse into grouped transcript cards, but pairs still fall through as expanded activity messages instead of using the same grouped collapsed chrome as larger clusters
- singleton low-signal activity items now use the same collapsed chrome as grouped activity with a singleton Show affordance
