...
- The normal entrypoint for preparing a worker-backed feature branch should be `bun run agent:prepare-worker-surface -- [--repo-profile <repo-profile>] <feature-branch-name>`, which should ensure or launch a worker, prepare the target repository on that worker, create the worker worktree from the repository profile base branch, run the repository profile install command, and print the canonical `start_command` for entering that worktree.
- The normal entrypoint for preparing a manager-hosted feature branch should be `bun run agent:prepare-manager-surface -- [--repo-profile <repo-profile>] <feature-branch-name>`, which should create the manager-hosted feature worktree from the repository profile base branch and print the canonical `start_command` for entering that worktree.
...
- When a swarm worker is used for development, the normal entrypoint for manager-side integration should be `bun run agent:merge-worker-feature -- [--repo-profile <repo-profile>] <feature-branch-name>`, which should apply the repository profile integration behavior: runtime repos merge through a manager integration worktree and push the base branch, while PR-to-staging repos push the refreshed feature branch and open the staging pull request.
- When a manager-hosted feature worktree is used for development, the normal entrypoint for manager-side integration should be `bun run agent:merge-manager-feature -- [--repo-profile <repo-profile>] <feature-branch-name>`, which should apply the same repository profile integration behavior from the manager-hosted worktree.
...
- Worker setup should run the repository profile install command in the worker-hosted feature-branch worktree before code changes begin.
...
- Runtime rollout should follow the repository profile deploy blueprint as the standard deploy path.
- Runtime rollout should call `bun run deploy-manager-runtime` for runtime repos after release promotion unless a more specific documented operator entrypoint supersedes it, while PR-to-staging repos should use their reviewed pull request promotion path and staged environment validation.
...
- PersistentWorkerTerminalWorkflow means worker-host development should use the `start_command` printed by `bun run agent:prepare-worker-surface -- [--repo-profile <repo-profile>] <feature-branch-name>` to enter a long-lived worker terminal in the prepared worktree for normal editing and verification loops instead of repeated manual ssh setup.
- WorkerPreflightVerification means a new worker-hosted feature branch is prepared by `bun run agent:prepare-worker-surface -- [--repo-profile <repo-profile>] <feature-branch-name>`, which should ensure worker readiness, prepare the target repository, create the worker worktree, run the repository profile install command, and produce a usable worker worktree entrypoint before implementation begins.
