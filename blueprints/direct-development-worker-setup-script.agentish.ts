...
const DirectDevelopmentWorkerSetupScript = define.system(
  "DirectDevelopmentWorkerSetupScript",
  {
    format: Agentish,
    role: "Standalone setup-and-test entrypoint for direct worker-local development",
  },
);
...
DirectDevelopmentWorkerSetupScript.contains(
  SubjectBlueprint,
  SectionMap,
  Section.concept,
  Section.scenarios,
...
DirectDevelopmentWorkerSetupScript.enforces(`
- This subject is a standalone setup-and-test script for direct worker-local development and must remain separate from the current \`prepare-worker-surface\` behavior.
...
DirectDevelopmentWorkerSetupScript.defines(`
- WorkerSetupScriptConfig means the complete CLI-derived or programmatic input object for the worker setup script.
...
- WorkerSetupPhaseResult means one phase-scoped status payload for a major step in the setup-and-test flow.
...
Section.concept.answers(
  "Why does this script exist? To provide one direct, repeatable setup-and-test entrypoint for worker-local development that prepares the worker, verifies the required credentials and tools, and proves the local browser workflow end to end without depending on SSH or the existing worker-surface bootstrap path.",
  "What is authoritative? The script is the authoritative orchestration for this specific setup-and-test flow. It owns the sequence from prerequisite verification through repo sync, dashboard startup, and the three browser verification passes. Durable worker-local credential surfaces remain the authoritative source for auth material, including the GitHub App credentials needed for repo access, while the current worker-surface setup path remains authoritative for worker-surface preparation but not for this script’s behavior.",
  "What must remain true? This subject stays separate from the current `prepare-worker-surface` behavior, does not redefine manager-worker architecture, and does not become a generic worker lifecycle blueprint. Its scope is narrow and non-negotiable: prepare the worker, validate the required auth and tool prerequisites, sync the repos to `origin/development`, start the dashboard on localhost, and validate the flow with direct `agent-browser`, Codex CLI, and Claude CLI runs that each save a screenshot.",
  "What are the core invariants? One script owns one worker-local direct-development setup-and-test flow. Repo state, dashboard startup, and browser verification are part of that one flow and should not be split across competing orchestrators. Any SSH-mediated agent execution, temporary bootstrap credential ownership, broad lifecycle management, or manager-side workflow expansion is out of scope for this blueprint.",
);
...
Section.scenarios.answers(
  ...
  "What do humans observe? The run is readable as named phases: prerequisite setup, repo preflight, repo synchronization, dashboard startup, direct browser verification, Codex verification, and Claude verification. Each phase either reports a machine-readable success result or fails with a stage-specific error, repo name, or readiness timeout rather than collapsing into a generic worker-setup error.",
  "What counts as success? The worker is prepared, each target repo is synchronized to the expected `origin/development` state, the canonical localhost dashboard is live, and the direct browser, Codex, and Claude verification paths have each produced screenshots from that same dashboard instance.",
...
Section.implementationPlan.answers(
  ...
  "What implementation choices remain closed? The script remains additive and isolated. It does not encode worker lifecycle management, manager-side feature-branch preparation, release or deploy behavior, or broader development-process architecture. The helper directory under `scripts/worker-setup-direct-development/` is optional and should be introduced only if the single entrypoint becomes too dense.",
  "What is the ideal file hierarchy? `scripts/worker-setup-direct-development.ts`; `scripts/worker-setup-direct-development.test.ts` if command parsing, prompt wiring, or artifact naming needs direct regression coverage; `scripts/worker-setup-direct-development/` only if the single entrypoint becomes too dense to keep readable.",
);
