...
- Full Development Process uses a swarm worker checkout as its active mutable implementation surface and does not use the manager host shared checkout as an editing surface.
- After creating or attaching the worker-hosted feature-branch worktree, the agent should merge any relevant upstream from `origin/development` or `origin/main` into that branch before continuing with implementation work.
