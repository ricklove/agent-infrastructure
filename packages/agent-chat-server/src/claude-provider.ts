...
export async function runClaudeTurn(
  ...
  const prompt = (async function* () {
    yield {
      ...
      message: {
        role: "user" as const,
        content: buildClaudeMessageContent(
          inputBlocks,
    ...
    for await (const rawMessage of runQuery) {
        ...
        if (
          ...
          if (textDelta) {
            ...
            callbacks.onAssistantDelta?.({
              ...
              turnId: currentTurnId || currentThreadId,
              itemId: String(
                message.uuid || `claude-delta-${streamEvent.index ?? 0}`,
              ),
              delta: textDelta,
            })
          }
      ...
      if (message.type === "result") {
        ...
        if (message.subtype === "error") {
          throw new Error(
            String(
              message.error || finalResultText || "Claude Agent SDK run failed",
            ),
          )
        }
      }
