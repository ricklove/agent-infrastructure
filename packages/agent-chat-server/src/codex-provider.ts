...
const codexReadyzUrl =
  process.env.AGENT_CHAT_CODEX_READYZ_URL?.trim() ||
  "http://127.0.0.1:8799/readyz"
const codexLogPath =
...
function describeCodexCommandExecution(
  ...
  if (commandValue && typeof commandValue === "object") {
    ...
    const executable =
      typeof commandObject.executable === "string" &&
      commandObject.executable.trim()
        ? commandObject.executable.trim()
        : typeof commandObject.program === "string" &&
...
function describeCodexCompletionIssue(
  ...
  const secondaryUsedPercent = numberFromUnknown(
  ...
  const completionPromise = new Promise<void>((resolve, reject) => {
    ...
    ws.addEventListener("message", (event) => {
      ...
      if (message.method === "account/rateLimits/updated") {
        const params = message.params ?? {}
        latestRateLimits =
          (params.rateLimits as CodexRateLimitsPayload | undefined) ?? null
        return
      }
