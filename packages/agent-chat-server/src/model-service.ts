...
function buildCatalogSnapshot(
  ...
  metadata: Pick<
    ...
    "source" | "fetchedAtMs" | "accountApiProvider" | "lastError"
  >,
): ClaudeModelCatalogSnapshot {
  const modelsByRef = Object.fromEntries(
    models.map((model) => [model.modelRef, model]),
  )
  return {
...
export function normalizeClaudeSessionModelRef(modelRef: string) {
  ...
  const fallbackMatch = fallbackClaudeModels.find(
    (model) =>
      model.modelRef === trimmed ||
      model.sdkValue === trimmed ||
      trimmed === `anthropic/${model.sdkValue}`,
  )
  if (fallbackMatch) {
