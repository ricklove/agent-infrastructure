...
export async function createHttpServer(repository: DocumentRepository) {
  ...
  return Bun.serve({
    ...
    async fetch(request, server) {
      ...
      if (
        url.pathname === "/api/agent-graph/open-board" &&
        request.method === "POST"
      ) {
      ...
      if (
        url.pathname === "/api/agent-graph/save-board-as" &&
        request.method === "POST"
      ) {
      ...
      if (
        url.pathname === "/api/agent-graph/add-board-document" &&
        request.method === "POST"
      ) {
        const payload = (await request.json()) as { path?: string }
        if (!payload.path) {
        ...
        const absoluteDocumentPath = resolve(
          WORKSPACE_ROOT,
          payload.path.trim(),
        )
        const nextDocumentPath = relative(
          dirname(repository.getBoardPath()),
        ...
        const nextBoardFile = {
          ...
          documents: [
            ...new Set([
              ...repository.getBoardFile().documents,
              nextDocumentPath,
            ]),
          ],
        }
