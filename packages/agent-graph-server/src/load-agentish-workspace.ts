...
function extractObjectDefinitions(args: {
  ...
  for (const objectMatch of source.matchAll(
    /const\s+([A-Za-z][A-Za-z0-9]*)\s*=\s*{([\s\S]*?)};/g,
  )) {
    ...
    for (const propertyMatch of objectBody.matchAll(
      ...
      const definitionIndex =
        (objectMatch.index ?? 0) + (propertyMatch.index ?? 0)
...
function extractRelationshipEdges(
  ...
  for (const match of source.matchAll(
    ...
    for (const targetReference of extractReferencedTargets(
      callArgs,
      knownReferences,
    )) {
...
async function loadSourceDocumentGraph(
  documentPath: string,
  relativePath: string,
): Promise<{
  document: SourceDocument
  ...
  for (const definition of definitionMap.values()) {
    const node = nodes.find(
      (candidate) =>
        candidate.id === toId(`${document.id}:${definition.reference}`),
    )
    if (node) {
...
export function normalizeWorkspaceState(
  ...
  const validPinnedNodeIds = new Set(
    nextLayers.flatMap((layer) =>
      layer.nodeIds.map((nodeId) => `${nodeId}::${layer.id}`),
    ),
  )
