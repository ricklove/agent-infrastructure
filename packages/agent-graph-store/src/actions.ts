...
function connect(store: AgentGraphStore): WebSocket {
  ...
  ws.addEventListener("open", () => {
    ws.send(
      JSON.stringify({ type: "client/hello" satisfies ClientMessage["type"] }),
    )
  })
...
function sendIntent(
  ...
  if (ws?.readyState === WebSocket.OPEN) {
    ws.send(
      JSON.stringify({ type: "client/intent", intent } satisfies ClientMessage),
    )
  }
}
  ...
  return {
    async openWorkspace(): Promise<void> {
      ...
      try {
        const response = await featureFetch(
          buildFeatureUrl(
            store.state$.connection.apiRootUrl.get(),
            "workspace",
          ),
        )
        if (!response.ok) {
    ...
    setLayerVisibility(layerId: string, visible: boolean): void {
      ...
      if (!visible && store.state$.activeLayerId.get() === layerId) {
        const fallbackLayerId =
          store.state$.graph
            .get()
            ?.layers.find((layer) => layer.id !== layerId && layer.visible)
            ?.id ?? null
    ...
    async addBoardDocument(path: string): Promise<void> {
      ...
      if (!response.ok) {
        throw new Error(
          `Add board document failed with status ${response.status}.`,
        )
      }
      const payload = (await response.json()) as GetWorkspaceResponse
      applySnapshot(store, payload.workspace, payload.graph, payload.diff)
