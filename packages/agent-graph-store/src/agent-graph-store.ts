...
export function findSelectedNode(
  ...
  return (
    state.graph.nodes.find((node) => node.id === state.selection.nodeId) ?? null
  )
}
...
export function findSelectedEdge(
  ...
  return (
    state.graph.edges.find((edge) => edge.id === state.selection.edgeId) ?? null
  )
}
...
export function queueIntent(
  state$: AgentGraphStore["state$"],
  intent: GraphIntent,
): void {
  state$.pendingIntents.set([
    ...state$.pendingIntents.get(),
    "intentId" in intent ? intent.intentId : intent.kind,
  ])
}
