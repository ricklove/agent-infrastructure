...
export function InspectorPanel({
  ...
  const multiSelectedNodes = selectedNodes.filter(
    (node) => node.kind === "semantic-node",
  )
    ...
    <section className="rounded-3xl border border-stone-800 bg-stone-900/80 p-3">
              ...
              {multiSelectedNodes.slice(0, 16).map((node) => (
                ...
                >
                  <button
                    ...
                    onMouseEnter={() => {
                      if (node.isVisible) {
                        actions.beginHidePreview(node.parentLayerId, [
                          node.sourceId,
                        ])
                      }
                    }}
      ...
      {selectedNode && !hasMultiSelection ? (
        <div className="mt-3 space-y-3">
          ...
          {selectedNode.kind === "semantic-node" ? (
            <div className="rounded-2xl border border-stone-800 bg-stone-950/70 p-3">
              <label
                htmlFor="semantic-node-connection-input"
                className="block text-xs uppercase tracking-[0.2em] text-stone-500"
              >
                Connect visible nodes
              </label>
              <input
                id="semantic-node-connection-input"
                className="mt-2 w-full rounded-2xl border border-stone-700 bg-stone-900 px-3 py-2 text-sm text-stone-100 outline-none placeholder:text-stone-500"
          ...
          {selectedNode.kind === "semantic-node" ? (
            <div className="rounded-2xl border border-stone-800 bg-stone-950/70 p-3">
              <div className="block text-xs uppercase tracking-[0.2em] text-stone-500">
                Expand hidden context
              </div>
              <p className="mt-2 text-xs leading-5 text-stone-400">
                Reveals every hidden incoming and outgoing neighbor connected to
                this node in the active layer.
              </p>
              <button
                type="button"
                onClick={() =>
          ...
          {selectedNode.kind === "semantic-node" ? (
            <div className="rounded-2xl border border-stone-800 bg-stone-950/70 p-3">
              ...
              <p className="mt-2 text-xs leading-5 text-stone-400">
                Removes this node from the current layer so it becomes hidden
                context again.
              </p>
              <button
                type="button"
                ...
                onMouseEnter={() =>
                  actions.beginHidePreview(selectedNode.parentLayerId, [
                    selectedNode.sourceId,
                  ])
                }
                onMouseLeave={actions.endHidePreview}
      ...
      {selectedEdge ? (
        <div className="mt-3 rounded-2xl border border-stone-800 bg-stone-950/70 p-3">
          ...
          <h3 className="mt-1 text-sm font-medium text-stone-100">
            {selectedEdge.label}
          </h3>
          <p className="mt-1 text-xs leading-5 text-stone-300">
