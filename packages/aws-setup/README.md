...
Each deployed manager uses three primary directories under `/home/ec2-user`: `runtime` for the checked-out runtime repo, `state` for bootstrap metadata, tokens, manifests, env files, SQLite, and session state, and `workspace` for checked-out repos and build workspaces. The worker launch script lives at `/home/ec2-user/runtime/packages/swarm-manager/scripts/launch-worker.sh`, the bootstrap context at `/home/ec2-user/state/bootstrap-context.json`, and worker runtime releases are tracked in `/home/ec2-user/state/worker-runtime-release.json`. The EC2 user data stays intentionally thin: it installs `git`, writes bootstrap context, clones `https://github.com/ricklove/agent-infrastructure.git` into `/home/ec2-user/runtime`, and hands off host provisioning to `/home/ec2-user/runtime/scripts/setup.sh`. That setup script is intended to be idempotent so it can be rerun after runtime updates. Systemd remains the boot-time supervisor, but its unit files are generated by that runtime-owned setup path and point at source-controlled scripts under `/home/ec2-user/runtime/packages/swarm-manager/scripts`; service configuration is loaded from `/home/ec2-user/state` rather than `/etc`. Launched workers follow the same layout, install Docker automatically, start a Bun telemetry agent, and are intended to be used as remote Docker hosts. The inline IAM policy on the manager role is scoped so it can only pass the worker role and only manage instances tagged for this swarm.
...
To destroy a non-default stack:

```bash
bun run destroy -- AgentSwarmAwsSetupV2
```
