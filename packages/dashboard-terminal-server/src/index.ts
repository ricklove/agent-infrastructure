...
const defaultCwd =
  process.env.DASHBOARD_TERMINAL_DEFAULT_CWD?.trim() ||
  "/home/ec2-user/workspace"
const allowedRoots = (
  process.env.DASHBOARD_TERMINAL_ALLOWED_ROOTS?.trim() ||
  "/home/ec2-user/workspace"
)
  .split(",")
  .map((r) => r.trim())
...
function createSession(opts: {
  ...
  if (!cwd)
    return {
      error: `Invalid or disallowed working directory: ${opts.cwd || defaultCwd}`,
    }
  ...
  const terminal = new Bun.Terminal({
    ...
    data(_terminal, data) {
      ...
      while (
        session.scrollbackBytes > maxScrollback &&
  ...
  for (const [id, session] of sessions) {
    if (session.closed) {
      ...
      if (
        ...
        !session.detachedAtMs &&
        now - session.lastActivityMs > reconnectWindowMs
      ) {
        sessions.delete(id)
      }
      continue
...
const _server = Bun.serve<TerminalSocketData>({
  port,
  fetch(req, server) {
    ...
    const closeMatch = path.match(
      /^\/api\/dashboard-terminal\/sessions\/([^/]+)\/close$/,
    )
    ...
    const snapshotMatch = path.match(
      /^\/api\/dashboard-terminal\/sessions\/([^/]+)\/snapshot$/,
    )
