...
export function PanZoomColorTargetSurface(props: { scenario: ScenarioKey }) {
  ...
  const [focusRect, setFocusRect] = useState<null | { left: number; top: number; width: number; height: number; nonce: string }>(null)
        ...
        <div className="flex items-center gap-1">
          {config.squares.map((square) => (
            <button
              key={square.key}
              type="button"
              onClick={() => setFocusRect({
                left: square.left,
                top: square.top,
                width: square.size,
                height: square.size,
                nonce: `${square.key}-${Date.now()}`,
              })}
...
function getScenarioConfig(scenario: ScenarioKey) {
  ...
  return {
    ...
    squares: [
      ...
      { key: "Blue", color: "#3b82f6", left: 1280, top: 420, size: 180 },
      { key: "Green", color: "#22c55e", left: 1640, top: 1240, size: 220 },
    ],
  }
}
