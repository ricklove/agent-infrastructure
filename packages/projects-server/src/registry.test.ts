...
describe("importLegacyInstallations", () => {
  test("copies legacy pem material into the managed installation directory", () => {
    ...
    const result = importLegacyInstallations(
      configRoot,
    ...
    if (!installation) {
      throw new Error("Expected installation to exist after persistence")
    }
    ...
    ).toContain(`GITHUB_APP_PEM=${installation.pemPath}`)
  })
  ...
  test("updates an existing imported installation to the managed pem path", () => {
    ...
    expect(result.registry.installations[0]?.pemPath).toBe(
      installationPemPath(configRoot, "5678"),
    )
  })
})
