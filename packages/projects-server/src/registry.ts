...
export function installationEnvPath(
  configRoot: string,
  installationId: string,
) {
...
export function installationPemPath(
  configRoot: string,
  installationId: string,
) {
...
export function persistInstallationEnv(
  configRoot: string,
  installation: InstallationRecord,
) {
  mkdirSync(installationDir(configRoot, installation.installationId), {
    recursive: true,
  })
  writeFileSync(
...
function syncManagedPem(
  ...
  if (
    readFileSync(managedPemPath, "utf8") !== readFileSync(sourcePemPath, "utf8")
  ) {
    copyFileSync(sourcePemPath, managedPemPath)
    return { pemPath: managedPemPath, changed: true }
  }
...
export function importLegacyInstallations(
  ...
  const installationsById = new Map(
    registry.installations.map((installation) => [
  ...
  for (const entry of readdirSync(configRoot, { withFileTypes: true })) {
    ...
    const sourcePemPath =
      env.GITHUB_APP_PEM?.trim() ||
      join(configRoot, entry.name, "github-app.pem")
    const accountLogin =
  ...
  if (changed) {
    registry.installations.sort((left, right) =>
...
export function readRegistry(
  ...
  const imported = importLegacyInstallations(
    configRoot,
