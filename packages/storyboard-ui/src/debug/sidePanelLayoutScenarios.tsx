...
export const sidePanelLayoutDebugDefinition: StoryboardDebugComponentDefinition = {
  ...
  scenarios: [
    ...
    {
      slug: "narrow-inspector",
      label: "narrow-inspector",
      description: "Narrow starting inspector with readable compact controls.",
      render: () => (
        <PanelLayoutScenario
          panels={[
            buildPanel(
              "inspector",
              "Compact inspector",
              "A tighter inspector start state that still keeps the form readable.",
              { initialWidth: 300, minWidth: 260 },
            ),
          ]}
          storageKey="storyboard.debug.sidePanel.narrow"
          title="Compact review board"
        />
      ),
      renderPreview: () => <SidePanelPreview title="Narrow inspector" />,
    },
    {
      slug: "wide-inspector",
      label: "wide-inspector",
      description: "Wide starting inspector for denser editing workflows.",
      render: () => (
        <PanelLayoutScenario
          panels={[
            buildPanel(
              "inspector",
              "Detailed inspector",
              "A wider inspector start state for dense multi-field editing.",
              { dense: true, initialWidth: 460, maxWidth: 720 },
            ),
          ]}
          storageKey="storyboard.debug.sidePanel.wide"
          title="Detailed editing board"
        />
      ),
      renderPreview: () => <SidePanelPreview title="Wide inspector" />,
    },
    {
      slug: "storyboard-editor-shape",
      label: "storyboard-editor-shape",
      description: "Closer to the intended storyboard editor layout and density.",
      render: () => (
        <PanelLayoutScenario
          panels={[
            buildPanel(
              "inspector",
              "Frame inspector",
              "Use this shape for storyboard editing: keep the board stable while the inspector resizes.",
              { dense: true, initialWidth: 380, minWidth: 300, maxWidth: 760 },
            ),
          ]}
          storageKey="storyboard.debug.sidePanel.editorShape"
          title="Storyboard editor board"
        />
      ),
      renderPreview: () => <SidePanelPreview title="Storyboard editor shape" />,
    },
    {
      slug: "max-width-stress",
      label: "max-width-stress",
      description: "Large right-side panel to verify the main area does not collapse poorly.",
      render: () => (
        <PanelLayoutScenario
          panels={[
            buildPanel(
              "analysis",
              "Wide analysis panel",
              "Stress the upper end of the resizable inspector while keeping the main board usable.",
              { dense: true, initialWidth: 560, minWidth: 320, maxWidth: 860 },
            ),
          ]}
          storageKey="storyboard.debug.sidePanel.maxWidthStress"
          title="Wide inspector stress board"
        />
      ),
      renderPreview: () => <SidePanelPreview title="Max width stress" />,
    },
    {
    ...
    },
    {
      ...
      render: () => (
        <div className="mx-auto flex h-full w-full max-w-[430px] bg-black">
          <PanelLayoutScenario
            mobileMode="nav"
            panels={[
              buildPanel(
                "navigator",
                "Navigator",
                "Mobile users should be able to switch between board navigation and editing without permanent side rails.",
                { sectionLabel: "Navigator" },
              ),
              buildPanel(
                "inspector",
                "Inspector",
                "Primary editing panel in mobile mode.",
                { dense: true, sectionLabel: "Inspector" },
              ),
              buildPanel(
                "notes",
                "Notes",
                "Human notes and agent tasks can live in a third mobile panel.",
                { sectionLabel: "Notes" },
              ),
            ]}
