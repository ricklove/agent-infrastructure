...
cd "${RUNTIME_ROOT}"
run_logged "build-worker-image.sh" "target=src/manager/build-worker-image.ts args=$*" \
  bun "${RUNTIME_ROOT}/packages/swarm-manager/src/manager/build-worker-image.ts" "$@"
