...
cd "${RUNTIME_ROOT}"
run_logged "wake-workers.sh" "target=src/manager/worker-power.ts action=wake args=$*" \
  bun "${RUNTIME_ROOT}/packages/swarm-manager/src/manager/worker-power.ts" --action wake "$@"
