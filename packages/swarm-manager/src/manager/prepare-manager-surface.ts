...
function usage(): never {
  console.log("Usage: bun run src/manager/prepare-manager-surface.ts -- <feature-branch-name>");
  process.exit(1);
}
...
function main() {
  ...
  if (runCommand(["git", "show-ref", "--verify", "--quiet", `refs/heads/${branchName}`], DEFAULT_REPO_PATH).exitCode === 0) {
  ...
  runChecked(
    ["git", "worktree", "add", "-b", branchName, worktreePath, `origin/${DEFAULT_BASE_BRANCH}`],
    DEFAULT_REPO_PATH,
  );
