...
async function main(): Promise<void> {
  ...
  const manifestPath =
    optionalOne(args, "manifest-path") ??
    DEFAULT_WORKER_RUNTIME_RELEASE_MANIFEST_PATH
  ...
  if (
    ...
    !bootstrapContext.workerRuntimeReleaseBucketName
  ) {
    throw new Error(
      "bootstrap context is missing worker runtime release bucket metadata",
    )
  }
  ...
  try {
    ...
    if (uploadResult.exitCode !== 0) {
      throw new Error(
        uploadResult.stderr.toString("utf8").trim() ||
          "runtime release upload failed",
      )
    }
