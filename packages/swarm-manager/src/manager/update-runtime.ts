...
function ensureGitCheckout(config: UpdateRuntimeConfig): void {
  ...
  runChecked(
    ["git", "pull", "--ff-only", "origin", config.repoRef],
    config.runtimeDir,
  )
}
...
function serviceUnitExists(name: string): boolean {
  return (
    existsSync(`/etc/systemd/system/${name}`) ||
    existsSync(`/usr/lib/systemd/system/${name}`)
  )
}
...
async function main(): Promise<void> {
  ...
  logStep(
    `setup.start runtimeDir=${config.runtimeDir} repoRef=${config.repoRef}`,
  )
