...
export function readWorkerImageProfileStore(
  ...
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Partial<WorkerImageProfileStore>;
    if (!parsed || typeof parsed !== "object" || !parsed.profiles || typeof parsed.profiles !== "object") {
      return { profiles: {} };
    }
    return { profiles: parsed.profiles as Record<string, WorkerImageProfileRecord> };
  } catch {
    return { profiles: {} };
  }
}
