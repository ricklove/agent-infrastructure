...
const workspacePersistenceRequestPath = resolve(
  stateRoot,
  "workspace-persistence-request.json",
)
const workspacePersistenceIncidentDir = resolve(
  stateRoot,
  "workspace-persistence-incidents",
)
const workspacePersistenceDebounceMs =
  Math.max(
    5,
    Number.parseInt(
      process.env.WORKSPACE_PERSISTENCE_DEBOUNCE_SECONDS ?? "15",
      10,
    ) || 15,
  ) * 1000
const workspacePersistencePollMs =
  Math.max(
    2,
    Number.parseInt(
      process.env.WORKSPACE_PERSISTENCE_POLL_SECONDS ?? "5",
      10,
    ) || 5,
  ) * 1000
const workspacePersistencePushRetryMs =
  Math.max(
    15,
    Number.parseInt(
      process.env.WORKSPACE_PERSISTENCE_PUSH_RETRY_SECONDS ?? "60",
      10,
    ) || 60,
  ) * 1000
...
function persistWorkspaceRepo(): { committed: boolean; pushed: boolean } {
  ...
  if (relativeTarget.startsWith("..")) {
    throw new Error(
      `agent chat data dir is outside workspace repo: ${agentChatDataDir}`,
    )
  }
  ...
  if (committed) {
    runGit(
      ["git", "commit", "-m", "Record agent chat workspace data"],
      repoRoot,
    )
  }
...
export async function runWorkspacePersistenceController(): Promise<void> {
  ...
  while (true) {
    ...
    const now = Date.now()
    const ready =
      request.flushNow === true ||
      now - request.requestedAtMs >= workspacePersistenceDebounceMs
    if (!ready) {
      await Bun.sleep(workspacePersistencePollMs)
      continue
    }
    ...
    if (
      lastFailureAtMs > 0 &&
      now - lastFailureAtMs < workspacePersistencePushRetryMs
    ) {
      await Bun.sleep(workspacePersistencePollMs)
      continue
    }
