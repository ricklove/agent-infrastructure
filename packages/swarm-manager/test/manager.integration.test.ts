...
beforeAll(async () => {
  ...
  await waitFor(async () => {
    const response = await fetch(`http://127.0.0.1:${managerPort}/health`).catch(
      () => null,
    );
...
describe("manager integration", () => {
  test("worker agents connect and are reported by the manager", async () => {
    ...
    const payload = await fetchJson<{
      workers: Array<{ workerId: string; status: string; lastMetrics: { containerCount: number } | null }>;
    }>(`http://127.0.0.1:${managerPort}/workers`);
  ...
  test("service registry resolves same-namespace first and falls back to root", async () => {
    await fetchJson(
      `http://127.0.0.1:${managerPort}/services/register`,
      {
        method: "POST",
        headers: {
  ...
  test("port leases are unique per worker and reusable after release", async () => {
    ...
    await fetchJson(
      `http://127.0.0.1:${managerPort}/ports/release`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
