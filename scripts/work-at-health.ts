...
Options:
  ...
  --fix                         Run explicit repair commands for failing checks
  --json                        Emit JSON only
  ...
  return {
    ...
    checks: raw.checks.map((entry, index) => {
      ...
      return {
        ...
        repairHint:
          typeof value.repairHint === "string" && value.repairHint.trim()
            ? value.repairHint.trim()
            : null,
        repairCommand:
          Array.isArray(value.repairCommand) &&
          value.repairCommand.every((part) => typeof part === "string")
            ? value.repairCommand.map((part) => part.trim())
            : null,
  ...
  const findings = profile.checks.map((binding) => {
    ...
    const envOverrides =
      check.execution.kind === "work-at"
        ...
        : undefined
    const result = runCommand(command, check.timeoutMs, envOverrides)
    let repairCommand: string[] | null = null
    ...
    if (
      options.fix &&
      result.exitCode !== 0 &&
      binding.repairCommand &&
      binding.repairCommand.length > 0
    ) {
      repairCommand =
        check.execution.kind === "work-at"
          ? [
              ...
              ...binding.repairCommand.map((part) => interpolate(part, params)),
            ]
          : binding.repairCommand.map((part) => interpolate(part, params))
      const repairResult = runCommand(
        repairCommand,
        Math.max(check.timeoutMs, 15_000),
        envOverrides,
      )
      repairAttempted = true
      repairExitCode = repairResult.exitCode
      repairStdout = repairResult.stdout.trim()
      repairStderr = repairResult.stderr.trim()
      finalResult = runCommand(command, check.timeoutMs, envOverrides)
    }
    ...
    return {
      ...
      repairHint: binding.repairHint,
      status:
        finalResult.exitCode === 0 ? ("healthy" as const) : ("unhealthy" as const),
...
function printReport(report: WorkspaceHealthReport): void {
  ...
  for (const finding of report.findings) {
    ...
    if (finding.status === "unhealthy" && finding.repairHint) {
      console.log(`  repair: ${finding.repairHint}`)
    }
    if (finding.repairAttempted) {
      console.log(
        `  repair-command: ${(finding.repairCommand ?? []).map(quote).join(" ")}`,
      )
      console.log(
        `  repair-result: ${finding.repairExitCode === 0 ? "ok" : finding.repairExitCode ?? "failed"}`,
      )
      const repairDetail = finding.repairStderr || finding.repairStdout
      if (repairDetail) {
        console.log(`  repair-detail: ${repairDetail}`)
      }
    }
    if (detail) {
